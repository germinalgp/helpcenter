	...
	if ($_SESSION['level'] == 1 || $_SESSION['level'] == 2) {
		echo '<html>
			...
			if ($anio == ''){
				...
				$tIncTargetTypeG1 = mysqli_query($connection, "SELECT c.DESCRIPCION, COUNT(*) FROM peticiones p, tipos_combos c WHERE p.ISSUE_TYPE = c.ID_COMBO GROUP BY (p.ISSUE_TYPE)");
				...
				//PARA GRAFICA 1 TOTAL
				$tIncTargetTypeG1 = mysqli_query($connection, "SELECT c.DESCRIPCION, COUNT(*) FROM peticiones p, tipos_combos c WHERE p.ISSUE_TYPE = c.ID_COMBO AND p.DATE >= '".$anio."-01-01 00:00:00' AND p.DATE <= '".$anio."-12-31 23:59:59' GROUP BY (p.ISSUE_TYPE)");
			...
			$mcIncTargetTypeG1->displayChart('Incidencias por tipo incidencia (AL A&#209;O) '.$anio,1,550,150,true);
			echo '<br/>';
			$mcIncTargetTypeG2 = new maxChart($data1);
			$mcIncTargetTypeG2->displayChart('Incidencias por tipo incidencia (AL MES) '.$anio,1,550,150,true);
			echo '<br/>';
			$mcIncTargetTypeG3 = new maxChart($data2);
			$mcIncTargetTypeG3->displayChart('Incidencias por tipo incidencia (AL DIA) '.$anio,1,550,150,true);
			echo '</div>
				<div id="footer">';
